...
export function hashCommandVariants(command: string): ReadonlySet<string> {
...
function collectCommandVariantHashes(command: string, hashes: Set<string>, depth: number): void {
...
function consumeShellEscape(state: ShellParseState, character: string): boolean {
  if (state.escaped) {
    state.word += character;
    state.escaped = false;
    return true;
...
function consumeShellQuote(state: ShellParseState, character: string): boolean {
...
function finishParsedWord(state: ShellParseState): void {
  if (state.word) {
...
function finishParsedCommand(state: ShellParseState): void {
  finishParsedWord(state);
...
function normalizeSimpleCommand(words: string[]): string[] {
  const firstCommandIndex = words.findIndex((word) => !/^[A-Za-z_][A-Za-z0-9_]*=/u.test(word));
  if (firstCommandIndex < 0) {
    return [];
  }
  const commandIndex =
    words[firstCommandIndex] === "exec" || words[firstCommandIndex] === "command"
      ? firstCommandIndex + 1
      : firstCommandIndex;
  return words.slice(commandIndex);
}

function readNestedShellCommand(words: string[]): string | undefined {
...
function addHashes(target: Set<string>, hashes: ReadonlySet<string>): void {
